...
const UserProfileContainer = () => {
  ...
  useEffect(() => {
    ...
    if (
      activeTab === 'liked' &&
      !likedPostsState.fetched &&
      !likedPostsState.loading
    ) {
      dispatch(getLikedPosts({ page: 1, reset: true }))
    }

    if (
      activeTab === 'saved' &&
      !savedPostsState.fetched &&
      !savedPostsState.loading
    ) {
      dispatch(getSavedPosts({ page: 1, reset: true }))
    }
  }, [
    activeTab,
    isOwnProfile,
  ...
  const TABS_CONFIG = {
    ownProfile: [
      ...
      },
      {
        ...
        label: 'Saved',
        icon: IconBookmark,
        count: userData?.savedPosts.length || 0,
      },
      {
        id: 'liked',
        label: 'Liked',
        icon: IconHeart,
        count: userData?.likedPosts.length || 0,
      },
    ],
    publicProfile: [
      {
  ...
  const isFollowing = useMemo(() => {
    ...
    return userData.followers.some(
  ...
  const handleApproveRequest = async (requesterId) => {
    ...
    await dispatch(approveFollowRequest(requesterId))
      .unwrap()
  ...
  const handleRejectRequest = async (requesterId) => {
    setProcessingRequestId(requesterId)
    await dispatch(rejectFollowRequest(requesterId))
      .unwrap()
      .finally(() => setProcessingRequestId(null))
  }
  ...
  const handleMessageClick = async () => {
    ...
    try {
      ...
      const result = await dispatch(
        createOrGetPrivateChat(userData?._id)
      ).unwrap()
            ...
            <div className="h-40 md:h-56 lg:h-64">
              <img
                src={
                  ...
                  `https://source.unsplash.com/random/1200x400/?abstract,${userData?.username}`
                }
                alt="Banner"
              ...
              <div className="flex flex-col md:flex-row md:items-end gap-4">
                ...
                >
                  <img
                    ...
                    className="w-full h-full object-cover rounded-3xl border-4 border-black/40 shadow-lg"
                  />
                  {userData?.isVerified && (
                    <motion.div
                      initial={{ scale: 0 }}
                      ...
                      className="absolute -top-2 -right-2 bg-gradient-to-tr from-blue-500 to-cyan-400 p-2 rounded-full shadow-lg"
                    >
                ...
                <div className="flex flex-wrap items-center gap-x-4 gap-y-2 mt-4 text-sm text-white/50">
                  {age && (
                    <span className="flex items-center gap-1.5">
                      <IconCake size={16} /> {age} years old
                    </span>
                  )}
                  {userData?.gender === 'male' && (
                    <span className="flex items-center gap-1.5">
                      <IconGenderMale size={16} /> Male
                    </span>
                  )}
                  {userData?.gender === 'female' && (
                    <span className="flex items-center gap-1.5">
                      <IconGenderFemale size={16} /> Female
                    </span>
                  )}
                  <span className="flex items-center gap-1.5">
              ...
              <div className="flex flex-col justify-start gap-4">
                <div className="flex justify-around bg-black/20 p-3 rounded-xl w-full">
                  <div className="text-center">
                    <div className="font-bold text-white text-lg">
                      {formatNumber(userData?.posts.length)}
                    </div>
                ...
                {!isOwnProfile ? (
                  ...
                  <button
                    ...
                    disabled={!userData?.followRequestsReceived?.length}
                    className="w-full flex items-center justify-center gap-2 py-2.5 rounded-lg text-sm font-semibold bg-white/10 text-white hover:bg-white/20 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    <IconUserPlus size={18} />{' '}
                    {userData?.followRequestsReceived?.length || 0} Follow
                    Requests
                  </button>
                )}
              </div>
                  ...
                  {!canViewContent ? (
                    ...
                    <>
                      ...
                      {activeTab === 'liked' && isOwnProfile && (
                        <>
                          {likedPostsState.loading &&
                          likedPostsState.posts.length === 0 ? (
                            ...
                            <>
                              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-3 xl:grid-cols-3 gap-2">
                                {likedPostsState.posts.map((post, index) => (
                                  <motion.div
                                    key={post._id}
                                    initial={{ opacity: 0, scale: 0.8 }}
                                    animate={{ opacity: 1, scale: 1 }}
                              ...
                              {/* Load More Button */}
                              {likedPostsState.pagination?.hasMore && (
                                  ...
                                  <button
                                    onClick={() =>
                                      dispatch(
                                        getLikedPosts({
                                          page:
                                            likedPostsState.pagination
                                              .currentPage + 1,
                                        })
                                      )
                                    }
                                    disabled={likedPostsState.loading}
                                    className="px-6 py-2 bg-white/10 hover:bg-white/20 rounded-lg text-white text-sm font-medium transition-colors disabled:opacity-50"
                                  >
                                    {likedPostsState.loading
                                      ? 'Loading...'
                                      : 'Load More'}
                      ...
                      {activeTab === 'saved' && isOwnProfile && (
                        <>
                          {savedPostsState.loading &&
                          savedPostsState.posts.length === 0 ? (
                            <div className="flex items-center justify-center h-64">
                              <div className="animate-spin h-8 w-8 border-4 border-white/30 border-t-white rounded-full"></div>
                            </div>
                          ) : savedPostsState.posts.length === 0 ? (
                            ...
                            <>
                              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-3 xl:grid-cols-3 gap-2">
                                {savedPostsState.posts.map((post, index) => (
                                  <motion.div
                                    key={post._id}
                                    initial={{ opacity: 0, scale: 0.8 }}
                                    animate={{ opacity: 1, scale: 1 }}
                              ...
                              {savedPostsState.pagination?.hasMore && (
                                <div className="flex justify-center mt-6">
                                  <button
                                    onClick={() =>
                                      dispatch(
                                        getSavedPosts({
                                          page:
                                            savedPostsState.pagination
                                              .currentPage + 1,
                                        })
                                      )
                                    }
                                    disabled={savedPostsState.loading}
                                    className="px-6 py-2 bg-white/10 hover:bg-white/20 rounded-lg text-white text-sm font-medium transition-colors disabled:opacity-50"
                                  >
                                    {savedPostsState.loading
                                      ? 'Loading...'
          ...
          >
            ...
            >
              <div className="flex items-center justify-between p-4 border-b border-white/10 flex-shrink-0">
                <h3 className="text-xl font-semibold text-white">
                  Follow Requests (
                  {userData?.followRequestsReceived?.length || 0})
                </h3>
                <button
                  onClick={() => setShowFollowRequestsModal(false)}
                  className="p-1 rounded-full text-white/50 hover:text-white hover:bg-white/10 transition-colors"
              ...
              <div className="p-2 space-y-2 overflow-y-auto">
                {userData?.followRequestsReceived &&
                userData.followRequestsReceived.length > 0 ? (
